...
// 匹配以 --- 开始和结束的 Front Matter 内容
const frontMatterRegex = /^---\s*\r?\n([\s\S]*?)\r?\n---\s*/;
const keywordsRegex = /^(title|date|updated|tags|keywords|categories):\s.*$/; // 关键字行的正则表达式
                    ...
                    fs.stat(filePath, (err, stats) => {
                        ...
                        if (stats.isDirectory()) {
                            ...
                            fs.readFile(filePath, 'utf-8', (err, content) => {
                                ...
                                // 先删除 Front Matter 部分（包括 --- 和它们之前的内容）
                                let newContent = content.replace(frontMatterRegex, '').trim();

                                // 如果没有 Front Matter，则删除包含关键字的行
                                newContent = newContent.split('\n').filter(line => !keywordsRegex.test(line)).join('\n').trim();
